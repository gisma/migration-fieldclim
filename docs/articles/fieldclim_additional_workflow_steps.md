# fieldClim: Anschlussworkflows und weitere Paketfunktionen
...
sind sie praktisch nützlich, und welche Einschränkungen muss man bei der
Anwendung beachten?

| Baustein | Leitfrage | Paketbereich |
|----|----|----|
| 0 | Wie arbeitet das Paket grundsätzlich? | `weather_station`, Default-Methoden, S3-Methoden |
| 1 | Wie werden Strahlungsreihen geprüft, ohne Messwerte still zu ersetzen? | `rad_*`, Messspalten, Zeitreihenprüfung |
| 2 | Wie kann Strahlung modelliert werden, wenn Messwerte fehlen oder als Plausibilitätsmaßstab gebraucht werden? | `sol_*`, `trans_*`, `terr_*`, `rad_*` |
| 3 | Wie werden Bodenparameter und Bodenwärmestrom behandelt? | `soil_*` |
| 4 | Welche Hilfsgrößen werden für Feuchte, Druck und Temperatur gebraucht? | `hum_*`, `pres_*`, `temp_*` |
| 5 | Wie werden Windprofil, Richardson-Zahl und Stabilitätsgrößen geprüft? | `turb_*`, `bound_*` |
...
Diese Vignette ist eine Paketlandkarte mit ausführbaren Mini-Beispielen.
...
Alle Bausteine verwenden denselben kleinen Lehrdatensatz. Der Datensatz
...
# Die Solar- und Strahlungsfunktionen können mit POSIXct-Zeitstempeln
# arbeiten. Die Zeitzone bleibt deshalb explizit gesetzt.
# Grundkontrollen.
nrow(caldern)
#> [1] 288
...
## Workflow-Baustein 0: Default-Methode oder weather_station-Methode?
...
## Workflow-Baustein 1: Strahlungsreihen prüfen und Arbeitsgröße festlegen
...
**Interpretation.** Dieser Baustein ist ein Datenvalidierungsworkflow.
...
# POSIXct bleibt hier bewusst erhalten; die aktuelle Paketfassung behandelt
# POSIXct/POSIXlt konsistent.
...
**Interpretation.** Dieser Baustein ist relevant, wenn
Strahlungskomponenten fehlen oder wenn geprüft werden soll, ob Messwerte
in einer plausiblen Größenordnung liegen. Modellierte Strahlung ist hier
keine automatische Ersatzmessung. Sie ist ein Vergleichsmaßstab: Stimmen
Tagesgang und Größenordnung grob, spricht das für eine konsistente
Zeitbasis und realistische Standortannahmen. Weichen Messung und Modell
stark ab, muss zuerst geprüft werden, ob Zeitzone, Oberflächentyp,
Geländeparameter, Sensorverarbeitung oder Strahlungskomponenten
unterschiedlich definiert sind. Technisch wichtig ist hier die
Zeitbasis: Die aktuelle Paketfassung behandelt POSIXct- und
POSIXlt-Zeitstempel konsistent. Entscheidend bleibt, dass die Zeitzone
explizit gesetzt ist, weil Sonnenstand und Strahlung vom lokalen
Zeitpunkt abhängen.
...
# Die Funktion wird vektorisiert auf die POSIXct-Zeitachse angewendet.
...
**Interpretation.** Der Bodenbaustein ist vor allem für zwei Situationen
nützlich: Entweder fehlt ein direkt gemessener Bodenwärmestrom, oder
Bodenfeuchte und Textur sollen als Szenario verändert werden. Ein aus
Temperaturgradient und Bodenparametern berechneter Wert ist aber nicht
dasselbe wie eine Wärmeflussplatte. Unterschiede sind deshalb erwartbar.
Für die Energiebilanz ist wichtig, die Vorzeichenkonvention
beizubehalten: In `fieldClim` bedeutet positiver `soil_flux`, dass Wärme
in den Boden geht und damit von der verfügbaren turbulenten Energie
abgezogen wird.

## Workflow-Baustein 4: Feuchte, Druck und Temperatur als Hilfsgrößen
...
**Interpretation.** Diese Hilfsgrößen sind Zwischenstufen, keine
eigenständigen Ergebnisflüsse. Absolute Feuchte, potentielle Temperatur,
Luftdichte und Verdampfungswärme gehen später in Gradientenauswertungen,
Bowen, Penman und Profilverfahren ein. Gerade deshalb sind sie wichtig:
Ein kleiner Einheiten- oder Skalierungsfehler in einer Hilfsgröße kann
später als großer Unterschied im Wärmefluss erscheinen.

## Workflow-Baustein 5: Windprofil, Richardson-Zahl und Stabilität
...
**Interpretation.** Dieser Baustein prüft, ob die Profilinformation
überhaupt tragfähig ist. Richardson-Zahl und Austauschgrößen hängen an
Temperaturunterschieden, Windunterschieden und Messhöhen. Wenn die
Windschere sehr klein ist oder Vorzeichen wechseln, werden
Stabilitätsaussagen empfindlich. Genau solche Situationen erklären
später, warum profilbasierte Wärmeflusswerte auffällig werden können.
Die Werte sind deshalb zuerst als Plausibilitäts- und Screeninggrößen zu
lesen, nicht als eigene Energiebilanz.

### 5.1 Schutzparameter und kritische Randfälle dokumentieren

Einige Methoden besitzen `cap`-Argumente oder geben für ungültige Fälle
kontrolliert `NA` zurück. Diese Schutzmechanismen sind keine
physikalische Lösung. Sie markieren oder begrenzen Situationen, in denen
Nenner nahe null, nicht endliche Zwischenwerte oder extrem empfindliche
Profilbedingungen auftreten.
...
**Interpretation.** Caps und Schutzregeln verändern nicht die Messdaten.
Sie verhindern, dass einzelne Nenner nahe null oder nicht endliche
Zwischenwerte unkontrollierte Ausreißer erzeugen. Das ist keine
fachliche Validierung und keine Reparatur des Zeitschritts. Ein
gekappter oder als `NA` behandelter Wert bleibt ein Hinweis, dass die
jeweilige Profil- oder Gradientensituation kritisch ist.
...
**Interpretation.** Dieser Block bleibt bewusst klein.
Grenzschichtgrößen sind kein Nebenprodukt, das man ohne Fragestellung
aus jedem Stationsdatensatz sinnvoll auswertet. Die Hilfefunktionen
zeigen die verfügbare Schnittstelle; ein belastbarer
Grenzschicht-Workflow braucht aber passende Eingangsdaten und eine
eigene Modellannahme.

## Workflow-Baustein 6: Objekt ausgeben, plotten und exportieren
...
Paket-Anschlussworkflows. Die zentrale Logik lautet:
