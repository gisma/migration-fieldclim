# Articles

### Concepts

- [fieldClim: The scientific
  background](https://gisma.github.io/migration-fieldclim/articles/fieldclim_theory.md):
- [fieldClim: formula
  reference](https://gisma.github.io/migration-fieldclim/articles/fieldclim_formula_reference_vignette.md):

### Tutorials & Workflows (EN)

- [fieldClim: Missing-data inspection and input
  checks](https://gisma.github.io/migration-fieldclim/articles/fieldclim_missing_data_en.md):
- [fieldClim: Heat-flux workflow with package
  methods](https://gisma.github.io/migration-fieldclim/articles/fieldclim_flux_workflow_en.md):
- [fieldClim: Follow-up workflows and additional package
  functions](https://gisma.github.io/migration-fieldclim/articles/fieldclim_workflow_steps_en.md):
- [fieldClim: Choosing fieldClim Heat-Flux Methods by Measurement
  Design](https://gisma.github.io/migration-fieldclim/articles/fieldclim_m2m_en.md):

### Tutorials & Workflows (DE)

- [fieldClim: Manuelle Datenprüfung für Energiebilanz und
  Wärmeflussmethoden](https://gisma.github.io/migration-fieldclim/articles/fieldclim_missing_data.md):
- [fieldClim: fieldClim-Workflow für
  Wärmeflussmethoden](https://gisma.github.io/migration-fieldclim/articles/fieldclim_flux_workflow.md):
- [fieldClim: Anschlussworkflows und weitere
  Paketfunktionen](https://gisma.github.io/migration-fieldclim/articles/fieldclim_workflow_steps.md):

### Deprecated

- [FieldClim Package Update
  Documentation](https://gisma.github.io/migration-fieldclim/articles/fieldclim_update2024.md):
- [fieldClim: Check radiation and soil heat
  flux](https://gisma.github.io/migration-fieldclim/articles/fieldclim_check_rad_soil.md):
