...
Eine systematische Auswahlhilfe für die Frage, welcher Rechenweg zu welcher Messarchitektur passt, findet sich in der Methodenauswahlseite [Choosing fieldClim Heat-Flux Methods by Measurement Design](fieldclim_m2m_en.html). Diese Vignette bereitet dafür die Mess- und Energiebilanzgrößen vor; die eigentliche Anwendung der Paketmethoden erfolgt in der zweiten Workflow-Vignette.
...
Der eigentliche Paketworkflow mit `build_weather_station()`, `inspect_weather_station_inputs()` und den Wärmeflussmethoden wird in der zweiten Vignette fortgesetzt. Diese erste Vignette bleibt bewusst bei Datenprüfung, Strahlungskomponenten, Bodenwärmestrom und verfügbarer Energie.


# Notation

Diese Vignette verwendet für die bodennahe Energiebilanz dieselbe Referenznotation wie die Methodenauswahlseite: `Q*` ist die Netto-Strahlung, `B` ist der Bodenwärmestrom, `A = Q* - B` ist die verfügbare Energie, `H` ist der fühlbare Wärmestrom, `LE` ist der latente Wärmestrom und `R_E` ist der Residual- oder nicht geschlossene Energieterm.

Die langwellige Strahlung wird davon getrennt notiert: `L_down`, `L_up` und `L*` bezeichnen langwellige Strahlungskomponenten. Dieses `L` ist nicht der fühlbare Wärmestrom.
...
Diese Residualformel wird hier nur als Energiebilanzlogik eingeführt. Die konkrete Schätzung von `H` und `LE` erfolgt erst in der zweiten Workflow-Vignette über die Paketmethoden.
...
Diese Vignette nutzt diese Paketebenen noch nicht zur Flussberechnung. Sie prüft nur die Eingangsgrößen, die später von `build_weather_station()` und den Wärmeflussmethoden verwendet werden.
...
Hier steht `L` für longwave radiation, nicht für fühlbare Wärme. Fühlbare Wärme wird in der Referenznotation dieser Vignette als `H` bezeichnet.
...
In diesem Datensatz wird `L_up` als vorhandene Messspalte verwendet. Es wird hier keine Oberflächentemperatur berechnet. Die paketinterne Modellfunktion `rad_lw_out()` würde für eine modellierte langwellige Ausstrahlung eine bereitgestellte `surface_temp` benötigen; diese Vignette rekonstruiert `L_up` aber nicht aus `surface_temp`.
...
In dieser Vignette werden `H`, `LE` und `R_E` noch nicht berechnet. Sie werden nur als Referenzgrößen eingeführt, damit klar ist, welche Bedeutung `Q*` und `B` für die zweite Workflow-Vignette haben.
...
Im Datensatz liegt aber zusätzlich bereits eine Spalte `rad_net` vor. Deshalb wird hier keine neue Netto-Strahlung als Ersatzwert erzeugt. Die Komponentensumme dient nur als Kontrollgröße: Sie prüft, ob `rad_net` und `K* + L*` dieselbe Bilanzebene beschreiben.
...
Die verfügbare Energie für fühlbaren Wärmestrom `H` und latenten Wärmestrom `LE` ist:
...
Bis hierher wurden die Messdaten und die zentralen Energiebilanzgrößen geprüft. Die Größen `Q_star`, `B` und `Q_minus_B` sind damit als Arbeitsterme vorbereitet. Die zweite Vignette überführt denselben Datensatz in ein `weather_station`-Objekt und nutzt darauf die Paketmethoden für `H` und `LE`.

Die Auswahl des passenden Rechenwegs hängt von Messarchitektur und Zielgröße ab. Dafür verweist diese Vignette auf die Methodenauswahlseite [Choosing fieldClim Heat-Flux Methods by Measurement Design](fieldclim_m2m_en.html).
