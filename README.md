...
This development branch adds a stable teaching workflow for microclimate energy-balance calculations.
...
* `as.data.frame.weather_station()` supports the current flat `weather_station` object structure.
...
* Penman failures in `turb_flux_calc()` are non-fatal.
* Penman surface-type handling was consolidated so common `fieldClim` surface types such as `field` can be mapped to Penman-compatible resistance classes.
